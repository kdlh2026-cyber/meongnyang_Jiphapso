	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		http.csrf((csrf) -> csrf.disable()) // CSRF 보호 비활성화
			...
			.authorizeHttpRequests(request -> request
					...
					.requestMatchers("/","/main","/loginForm","/memberInsertForm","/memberInsert","/hamburger_menu","/animal_loading","/search","/autocomplete","/allSearch").permitAll() // 루트(/)는 모두 허용
					...
					.requestMatchers("/community/**","/comm_search","/autocomplete").permitAll()
					.requestMatchers("/favorite/**", "/cart/**","/products/**").permitAll() // 장바구니/관심상품 AJAX 액션은 회원·비회원 둘 다 호출하므로 인증 필터 없이 허용
