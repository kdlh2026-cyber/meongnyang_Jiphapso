...
public class CartController {
    ...
    /** 옵션 변경 모달용 - 해당 상품의 옵션 전체 목록 (색상/사이즈/가격/재고) */
    @RequestMapping(value = "/cart/options", method = RequestMethod.GET)
    @ResponseBody
    public ApiResponse<List<Map<String, Object>>> optionList(@RequestParam("pNo") Long pNo) {
        return ApiResponse.ok(cartService.getOptionListByProduct(pNo));
    }
    /** 옵션 변경 (연필 아이콘 -> 옵션 변경 모달에서 "변경" 클릭) */
    @RequestMapping(value = "/cart/{caNo}/option", method = RequestMethod.PUT)
    @ResponseBody
    public ApiResponse<CartDTO> updateOption(@PathVariable("caNo") Long caNo, @RequestBody Map<String, Object> body,
                                              HttpSession session, HttpServletRequest request, HttpServletResponse response) {
        try {
            Long mNo = loginMemberNo(session);
            String guestToken = (mNo == null) ? GuestTokenUtil.resolveGuestToken(request, response) : null;
            Long oNo = Long.valueOf(String.valueOf(body.get("oNo")));
            int quantity = body.get("quantity") != null ? Integer.parseInt(String.valueOf(body.get("quantity"))) : 1;
            CartDTO result = cartService.changeOption(caNo, oNo, quantity, mNo, guestToken); // 소유자 검증 후 옵션 변경
            return ApiResponse.ok("옵션을 변경했어요", result);
        } catch (IllegalArgumentException | IllegalStateException e) {
            return ApiResponse.fail(e.getMessage());
        }
    }
