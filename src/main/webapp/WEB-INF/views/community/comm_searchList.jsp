...
<%@ include file="/WEB-INF/views/hamburger_menu.jsp" %>
    ...
    <div class="community_wrap">
        ...
        <div class="filter_count_row">
            <div class="sub_filter">
                <c:if test="${empty param.comm_type or param.comm_type eq 'QNA' or param.comm_type eq '라운지'}">
                    <select name="sort" onchange="location.href='/community/search?keyword=${param.keyword}&amp;comm_type=${empty param.comm_type ? '' : param.comm_type}&amp;sort='+this.value+'&amp;comm_pet_type=${param.comm_pet_type}'">
                    ...
                    <a href="/community/search?keyword=${param.keyword}&amp;comm_type=${empty param.comm_type ? '' : param.comm_type}&amp;comm_pet_type=" class="${empty param.comm_pet_type ? 'active' : ''}">전체</a>
                    <a href="/community/search?keyword=${param.keyword}&amp;comm_type=${empty param.comm_type ? '' : param.comm_type}&amp;comm_pet_type=강아지" class="${param.comm_pet_type eq '강아지' ? 'active' : ''}">강아지</a>
                    <a href="/community/search?keyword=${param.keyword}&amp;comm_type=${empty param.comm_type ? '' : param.comm_type}&amp;comm_pet_type=고양이" class="${param.comm_pet_type eq '고양이' ? 'active' : ''}">고양이</a>
                    <a href="/community/search?keyword=${param.keyword}&amp;comm_type=${empty param.comm_type ? '' : param.comm_type}&amp;comm_pet_type=소동물" class="${param.comm_pet_type eq '소동물' ? 'active' : ''}">소동물</a>
                    <a href="/community/search?keyword=${param.keyword}&amp;comm_type=${empty param.comm_type ? '' : param.comm_type}&amp;comm_pet_type=기타" class="${param.comm_pet_type eq '기타' ? 'active' : ''}">기타</a>
                </c:if>
       ...
       <!-- 6. 게시글 카드 목록 영역 -->
        ...
        <!-- 페이징 네비게이션 (검색 유지 파라미터 반영) -->
        <div class="pagination">
            <c:if test="${startPage > 1}">
                <a href="/community/search?keyword=${param.keyword}&comm_type=${param.comm_type}&comm_pet_type=${param.comm_pet_type}&comm_category=${param.comm_category}&sort=${param.sort}&page=${startPage - 1}">PREV</a>
            </c:if>
            
            <c:forEach var="i" begin="${startPage}" end="${endPage}">
                <a href="/community/search?keyword=${param.keyword}&comm_type=${param.comm_type}&comm_pet_type=${param.comm_pet_type}&comm_category=${param.comm_category}&sort=${param.sort}&page=${i}" 
                   class="${pageNum eq i ? 'active' : ''}">${i}</a>
            </c:forEach>
            
            <c:if test="${endPage < totalPages}">
                <a href="/community/search?keyword=${param.keyword}&comm_type=${param.comm_type}&comm_pet_type=${param.comm_pet_type}&comm_category=${param.comm_category}&sort=${param.sort}&page=${endPage + 1}">NEXT</a>
            </c:if>
        </div>
